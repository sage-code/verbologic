...
## Dictionary search — commit-on-Enter + filter button (dictionary-wide translation search)
- [x] `roadmap/layouts/DictionaryToolbar.vue`: the query is a local draft — NOTHING filters while typing; it commits on **Enter** (`@keydown.enter.prevent`) or the native `@search` (covers the ✕ clear), or by clicking the new **filter button** right after the field (`FunnelIcon`, same `h-9 w-9` round shell as prev/next/play/loop, accent-filled + `aria-pressed` while active). Clicking the filter toggles the mode AND applies the draft; the placeholder swaps (`dictionary.letter_filter` ↔ `dictionary.translation_filter`); Next/Prev stop walking topics while results are shown
- [x] Two modes: filter **off** = today's diacritic-folded PREFIX on `term` in the open topic (the letter scanner, unchanged); filter **on** = SUBSTRING on the **rendered translation** across the WHOLE dictionary — in the Romanian dictionary with English on the top bar you type `chair` and find `Adu scaunul.` (C3T01). The mode persists across topics (like Repeat); the query resets on topic open/close; clearing the query returns to the topic table
- [x] Build: `media-index` now emits `public/data/media/<section>/search.json` — every record of the section in one trimmed payload (`id/entity_id/topic/lang/term/names/ipa/context/media.url`; 532 records ≈ 212 KB) for both target languages; the store filters by the track language; loaded lazily via `useMedia().fetchSearch()` only when the translation filter is used
- [x] `roadmapStore`: `letterQuery` → `dictionaryQuery` + `setDictionaryQuery`, `searchInTranslation` + `setSearchMode`, `searchActive`, `searchResults`, `loadingSearch` + `ensureSearchIndex()`; `dictionaryRows` serves the hits in search mode (pagination/Play-page/Repeat/learned all work on them); `MediaRow` (types/media.ts) is the shared row contract both payloads satisfy
- [x] `DictionaryLayout`: results take precedence in the mode machine; the banner reads "Search results — N"; each hit's File ID cell gains a clickable topic chip (`openTopic(r.topic)` → jumps into that word's topic and clears the search); the loading state covers the search payload; gloss + search share `glossName` (`src/lib/dictionarySearch.ts`) so the matched text is always exactly the displayed text; query/mode changes stop the autoplay
- [x] Locales (en/ro): `dictionary.translation_filter` `filter_translation` `search_results` `open_topic`
- [x] Docs: `media/readme.md` §3 (search.json output) · `MAINTENANCE.md` §8 (dictionary playback state row)
- [x] Verified: `vue-tsc` green · `validate-data` OK · `media-index` emits search.json (532 records) · `temp/verify_dictionary_search.mjs` 38/38 PASS (functional: fold/gloss/mode matching against the real payload + static wiring)

## Lectures & stories live — Nuxt Content authoring + dictionary restructure
- [x] Track-aware `roadmapStore`: `SECTION_BY_TRACK` + `init(lang, track, …)` — sidebar/counts/ids/payload caches are section-scoped and reset per track+lang (the store was hardcoded to `library/dictionary` before)
- [x] Empty scaffolding hidden: `populatedTopics` / `visibleChapters` (lists, sidebar, topic walks, Fuse hits) — 114 empty dictionary topics and the C9–C12 ghost chapters stop cluttering the UI; deep links to empty topics still resolve
- [x] `@nuxt/content` v3 (collections + zod front-matter schemas in `content.config.ts`, `sqliteConnector: 'native'` — no native dep on Node 24): one Markdown doc per explanation locale at `content/lectures/<trackLang>/<TOPIC>/<ID>/<locale>.md`, English canonical; stories collection pre-declared (Phase 4, target-language canonical)
- [x] Lecture pages are **prerendered** (`nuxt.config.ts` enumerates the content tree → `/learn/<trackLang>/lectures/<TOPIC>/<ID>/<locale>`): content queries run at build time — verified no `_content` client DB dump in `.output/public`, so the WASM SQLite never ships
- [x] `media-index` content overlay: front matter → topic payloads (localized titles → `names`, per-locale `content` review meta, `related` ids → embedded rows); `content/**` added to the build fingerprint
- [x] `run lecture` tooling (`scripts/lecture.mjs`): `new` (doc + pending video manifest + sidebar item) · `status` (the review queue; exits 1 on stale) · `translate <id> --lang <l>` (scaffold/refresh front matter only — never body text) · `review` (draft → reviewed + timestamp, refuses stale without --force)
- [x] `LectureLayout` (lecture cards over the shared word table) · `LecturePlayer` video card (pending = coming-soon) · MDC `::callout` / `::term` components · `MediaViewer` mime-aware (video/img/audio) · first lecture authored end-to-end: `content/lectures/ro/C1T01/lecture_alphabet/{en,ro}.md` (both draft — review pending)
- [x] Dictionary restructure: **127 letter/sound ids moved** (`library/dictionary C1T01–T03 → library/lectures C1T01–T03` — same code space, so R2 keys + entity_ids + progress untouched) and **pinned** (`pins.json`, honored by `archive-topics.mjs --apply-sidebar`); the dictionary is now the expression corpus (405 records: word 150 · question 135 · imperative 75 · sentence 30 · greeting 15); `/learn/en/dictionary` retired (EN was 100% alphabet) — EN's live track is Lectures; `gitignore` `lib/` anchored to `/lib/` (it was swallowing `src/lib/`)
- [x] Chapter rail rule (`RoadmapSidePane.vue` + `src/lib/roadmapRail.ts`): the rail is a **self-sized** panel — `self-start` cancels the grid's `align-items: stretch`, so its height is its own content capped at the viewport (`lg:max-h-[calc(100dvh-5rem)]`), sticky under the header (`lg:sticky lg:top-14`, the same offset token as the dictionary's sticky table heads) and the chapter list scrolls **inside** the rail (`lg:overflow-y-auto lg:overscroll-contain`); the page content keeps the browser scroll. Fixes the dictionary rail stretching to the table's height (the old `lg:flex lg:flex-col` + `lg:flex-1` behaviour, documented as intended) — all three layouts render the same `RoadmapSidePane`, so the rule lives in exactly one place
- [x] Verified: `vue-tsc` green · `validate-data` OK (content + pins integrity) · `nuxt generate` 82 routes · lecture prose prerendered in EN + RO · `temp/verify_lectures_pipeline.mjs` 46/46 PASS
...
## Topic layouts — the pane is the TOPIC's, never the track's
- [x] Model: every sidebar topic declares `layout` ∈ `table · article · gallery` (`src/types/sidebars.ts` `TOPIC_LAYOUTS` / `TopicLayout`); the open topic's pane is picked by THIS field in ANY track (Dictionary · Lectures · Stories) — strict one-topic-one-layout, no mixed topics
- [x] Backfill: `temp/backfill-topic-layouts.mjs` (idempotent) stamped all 337 topics — `table` default, `article` when the topic holds a content doc id; `gallery` is assigned by hand when image content lands
- [x] C1T01 split: `library/lectures` C1T01 = the 104 letter rows (`table`, manifests/R2 keys/progress untouched) + new **C1T01A** = the alphabet article (`article`, `lecture_alphabet` moved with its manifest and docs — pending video, so zero R2 churn); display order preserved (C1T01 → C1T01A → C1T02…)
- [x] Runtime: `RoadmapShell` is now the shared frame (store init, deep links, `PROGRESS_KEY`, the credit/chapter meters for EVERY track, the Chapters TOC toggle + `ChaptersTable.vue`) — the main pane is `TOPIC_LAYOUTS[store.topicLayout]` (registry in the shell, not per-track); retired `DictionaryLayout` / `LectureLayout` / `TopicsLayout` / `RoadmapWordTable` / `RoadmapPlayerBar`
- [x] New panes: `roadmap/layouts/TopicTable.vue` (the dictionary table + toolbar, now the `table` pane) · `TopicArticle.vue` (doc cards → prerendered pages, strictly prose) · `TopicGallery.vue` (image cards — image · term · gloss · learned toggle)
- [x] Articles anywhere: `content.config.ts` = one track-agnostic `articles` collection (front matter `track` + `article: <id>`; `run lecture` scaffolds it); the doc route is `src/pages/learn/[locale]/[track]/[topic]/[id]/[doc].vue` (404s unknown track ids); `nuxt.config.ts` enumerates doc routes for every `content/<track>`; `media-index.mjs` synthesizes records for content-only article ids (`kind: 'article'`, counts via a second pass) and the canonical rule is per-track (`EN_CANONICAL`: lectures → en, others → the track language)
