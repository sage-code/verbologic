"""Scaffold the gallery repository skeleton (binary folders + readme placeholders).

Three-layer content model (see gallery/readme.md):
  1. Sidebars (structure)   — src/data/sidebars/…/sidebar.json (build-inlined; NOT scaffolded here)
  2. Gallery (repository)   — media files, each with its own sibling <ID>.json manifest
  3. Location config        — src/data/gallery.config.json (R2 root + section → path)

This script creates only the repository's binary folder skeleton: one folder per
configured section path (from src/data/gallery.config.json), with `audio/`
...
Run from the repo root:  .venv/Scripts/python.exe scripts/scaffold-gallery.py
...
CONFIG = ROOT / "src" / "data" / "gallery.config.json"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Create the gallery repository skeleton (binary folders + readme placeholders)."
    )
    parser.add_argument(
    ...
    if not CONFIG.exists():
        parser.error("src/data/gallery.config.json is missing — it maps sections to R2 paths")
    ...
    for section_path in section_paths:
        ...
        if media in LANGUAGE_PARTITIONED:
            for lang in locales:
                create(GALLERY / section_path / lang, f"{lang} {media} files")
        else:
            create(GALLERY / section_path, f"{media} files")
